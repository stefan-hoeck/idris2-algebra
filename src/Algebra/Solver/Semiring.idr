...
eval r (Lit v)   = v
eval r (Var x y) = x
eval r (x + y)   = eval r x `m` eval r y
eval r (x * y)   = eval r x `p` eval r y
...
norm :
  ...
  -> Expr a as
  -> Sum a as
norm r (Lit n)   = [T n one]
norm r (Var x y) = [T o $ fromVar y]
norm r (x + y)   = add p (norm r x) (norm r y)
norm r (x * y)   = mult p m (norm r x) (norm r y)
...
normalize :
  ...
  -> Expr a as
  -> Sum a as
normalize r isZ e = normSum isZ (norm r e)
